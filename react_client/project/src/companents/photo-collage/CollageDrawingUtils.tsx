...
interface DrawCollageParams {
  ...
  borderColor: string
  polaroidText: string
  drawImageMaintainAspect: (x: number, y: number, width: number, height: number) => void
  drawBorder: (x: number, y: number, width: number, height: number) => void
}

// פונקציות ציור מתקדמות - ~150 שורות
export const drawCollageLayout = ({
  ctx,
  layout,
  canvasWidth,
  canvasHeight,
